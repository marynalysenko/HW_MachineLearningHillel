...
##############################     Блок построения метрик классификации      ##############################
    y_true = []
    y_pred = []

    model.eval()# переключаем модель в режим оценки

    # Собираем правильные метки классов и предсказания для каждого элемента тестовой выборки
    with torch.no_grad(): # отключаем вычисление градиентов для ускорения расчетов
        for features, target in test_loader:
            features, target = features.to(device), target.to(device)
            output = model(features)
    ...
    for c in unique_classes:
        class_size = np.sum(y_true == c)
        print("Class", c, "size:", class_size)
    ...
    # Print general accuracy, precision, recall and F1-score
    print(f"General accuracy: {accuracy:.4f}")
    print(f"General precision: {np.mean(precision):.4f}")
    print(f"General recall: {np.mean(recall):.4f}")
    print(f"General F1-score: {np.mean(f1_score):.4f}")

    # Print accuracy, precision, recall and F1-score for each class
    for i in range(len(unique_classes)):
        print(f"Class {i} accuracy: {cm[i, i] / np.sum(cm[i]):.4f}")
